...
class DeepCrackTrainer(nn.Module):
    def __init__(self, model):
        ...
        # self.vis = Visualizer(env=cfg.vis_env) #Tamim: off visdom
        self.model = model

        self.saver = Checkpointer(cfg.name, cfg.saver_path, overwrite=True, verbose=True, timestamp=True,
                                  max_queue=cfg.max_save) ## Tamim: made overwrite True to overwrite one after another.
        ...
        self.mask_loss = nn.BCEWithLogitsLoss(reduction='mean',
                                              pos_weight=torch.tensor([cfg.pos_pixel_weight], dtype=torch.float32, device='cpu'))  ## # Tamim: changed cpu from cuda
    ...
    def acc_op(self, pred, target):
        ...
        mask_pos_acc = pred_mask[mask > 0.00000].eq(mask[mask > 0].view_as(pred_mask[mask > 0])).sum().item() / mask[
            mask > 0].numel()
