...
def readIndex(index_path, shuffle=False):
    ...
    with open(index_path, 'r') as file_to_read:
        while True:
            ...
            # print(item, "item in the line") ## Tamim: printed the item to see the file path
            img_list.append(item)
            # print(img_list) ## tamim: printed to see if any issue for index error. 
    file_to_read.close()
    if shuffle is True:
...
class dataReadPip(object):
    ...
    def __init__(self, transforms=None):
        transforms = [] ## Tamim: added to make sure it accepts touple.
    ...
    def __call__(self, item):
        ...
        if len(lab.shape) != 2: 
           ...
           ## tamim: pass the transform
        for transform in self.transforms:
            img, lab = transform(img, lab)
