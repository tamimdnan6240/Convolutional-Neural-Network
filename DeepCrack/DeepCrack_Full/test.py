...
def test(test_data_path="data/train_path_index.txt",
         save_path='deepcrack_results/',
         pretrained_model='./checkpoints/DeepCrack_CT260_FT1/checkpoints/DeepCrack_CT260_FT1_epoch(48)_0000005_2023-04-04-23-35-31.pth', ):
    ...
    with torch.no_grad():
        ...
        # normalize predicted values to [0,1]
         ...
         test_pred = (test_pred * 255).to(torch.uint8)

         ## normalize and scale lab tensors
         lab_norm = (lab - lab.min()) / (lab.max() - lab.min())

         # scale to [0,255] and uint8
         lab_scaled = (lab_norm * 255).to(torch.uint8)
